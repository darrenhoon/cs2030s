...
public class LazyList<T> {
    ...
    public <R> LazyList<R> map(Function<T,R> f) {
        LazyList.mapCounter = LazyList.mapCounter + 1;
        System.out.println("map called: " + LazyList.mapCounter);
        if (this.isEmpty())
            return LazyList.makeEmpty();
        else
    ...
    public LazyList<T> filter(Predicate<T> pred) {
        LazyList.filterCounter = LazyList.filterCounter + 1;
        System.out.println("Filter called: " + LazyList.filterCounter);
        if (this.isEmpty())
            return LazyList.makeEmpty();
