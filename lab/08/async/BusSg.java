...
class BusSg {
    ...
    public static CompletableFuture<Set<BusService>> getBusServices(BusStop stop) {
       return BusAPI.getBusServicesAt(stop.getStopId())
           .thenApply(stops -> BusSg.getBusServicesBody(stops));
       }

    public static Set<BusService> getBusServicesBody(String busStop) {
        Scanner sc = new Scanner(busStop);
    ...
    public static CompletableFuture<BusRoutes> findBusServicesBetween(BusStop stop, String searchString) {
        try {
            return getBusServices(stop)
                .thenApply(bs -> bs.stream().collect(Collectors.toMap(service -> service, service -> service.findStopsWith(searchString))))
                .thenApply(validService -> new BusRoutes(stop, searchString, validService));
