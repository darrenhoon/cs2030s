...
class BusRoutes {
  ...
  public CompletableFuture<String> description() {
    ...
    for (BusService service: services.keySet()) {
      ...
      result = result.thenCombine( stops.thenApply(busStop -> describeService(service, busStop).join())
              , (x,y) -> x + y);
    }
    return result;
  }
  ...
  public CompletableFuture<String> describeService(BusService service, Set<BusStop> stops) {
    ...
    return CompletableFuture.completedFuture(stops.stream()
