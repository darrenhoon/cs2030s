...
class BusService {
    ...
    public CompletableFuture<Set<BusStop>> getBusStops() {
        return BusAPI.getBusStopsServedBy(serviceId).thenApply(this::getBusStopsBody);
    }

    public Set<BusStop> getBusStopsBody(String bus) {
    ...
    public CompletableFuture<Set<BusStop>> findStopsWith(String name) {
        return getBusStops().thenApply(bs -> findStopsWithBody(bs, name)); 
    }
    
    public Set<BusStop> findStopsWithBody(Set<BusStop> bs,String name) {
        Set<BusStop> set = bs
            .stream()
